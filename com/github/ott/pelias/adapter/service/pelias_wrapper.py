...
class PeliasWrapper(object):
    ...
    def wrapp(
        ...
        if geo_utils.is_coord(text):
            ...
            ll = geo_utils.xy_to_url_param_str(
                x, y, x_name="point.lon", y_name="point.lat", check_lat_lon=True
            )
    ...
    def is_wrong_city_bug(cls, response):
        ...
        if (
            features
            and len(features) == 1
            and pelias_json_queries.is_region_record(features[0])
        ):
        ...
        if cls.has_features(pelias_json):
            ...
            for i, f in enumerate(pelias_json.get("features")):
                ...
                # step 5: Post Office ... add zipcode to label
                    ...
                    rename = pelias_json_queries.append3(
                        name, "Post Office", zipcode, sep1=" "
                    )
